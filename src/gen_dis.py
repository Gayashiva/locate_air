                ...
                for rh in da.rh.values:
                    for v in da.v.values:
                        da.sel(hours=hour, temp=temp, rh=rh, v=v).data += autoDis(
                            **param_values, time=hour * 100, temp=temp, rh=rh, v=v
                        )

        da.to_netcdf("data/discharges.nc")

    else:
        da = xr.open_dataarray("data/discharges.nc")
        plt.figure()
        ax = plt.gca()
        da.sel(temp=slice(-15, None), rh=80, v=2).plot(cmap="RdBu")
        plt.legend()
        plt.grid()
        plt.savefig("figs/temp_sun.jpg")
